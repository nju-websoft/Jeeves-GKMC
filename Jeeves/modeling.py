...
class HMRS_Model(PreTrainedBertModel):
    def __init__(self, config, num_choices=4,
                 max_seq_length=128,
                 ere_num=200, sre_num_train=10, sre_num_test=5, ire_num=2, index_map=None, tokenizer=None):
        super(HMRS_Model, self).__init__(config)

        self.tokenizer = tokenizer

        self.ere_num = ere_num
        self.sre_num_train = sre_num_train
        self.sre_num_test = sre_num_test
        self.ire_num = ire_num
        ...
        self.W_3 = nn.Linear(self.ere_num, self.ere_num)
        self.w_4 = nn.Linear(self.ere_num, 1)
    ...
    def forward(self, input_ids, input_mask, segment_ids, q_ids=None, answers=None, have_negative_word=None,
                ...
                do_train=True, do_eval=False, do_pre_finetune=False, ere=True, sre=True, ire=True, p=False):
        ...
        if do_pre_finetune is True:
            ...
            if do_train:
                ...
                return u_l
        if do_train:
            sre_num = self.sre_num_train
        else:
            sre_num = self.sre_num_test
        ...
        if do_eval:
            ...
            p_ids, paragraphs, bm25_feature = self.first_fillter(paragraphs, p_ids, bm25_feature,
                                                                 ...
                                                                 batch_size, ir_size, self.ere_num,
                                                                 self.max_seq_length)

        bm25_feature = bm25_feature.view(batch_size * self.num_choices, self.ere_num, seq_len)
        ...
        z_l_sorted, z_l_idx = torch.sort(z_l, dim=1, descending=True)  # batch_size * self.num_choices, ere_num
        z_l_sorted = z_l_sorted.unsqueeze(dim=2)
        z_l_sorted = z_l_sorted.squeeze()
        score_ere = self.W_3(z_l_sorted)
        score_ere = self.tanh(score_ere)
        score_ere = self.w_4(score_ere)
        score_ere = score_ere.view(-1, self.num_choices)
        score_ere_soft = torch.softmax(score_ere, dim=1)

        # print('z_l:', z_l.shape)

        p_input_ids, p_input_mask, p_segment_ids, \
        p_paragraph_range_subword, p_scenario_range_subword, \
        p_question_range_subword, p_option_range_subword, p_labels_sre \
            = self.get_p_sre(z_l, p_labels, paragraphs,
                             scenarios, questions, options, batch_size, sre_num, word_set, word_weights_soft)

        p_labels = p_labels.view(-1, sre_num)
        ...
        u_l = u_l.view(-1, sre_num)
        score_sre = torch.sum(u_l, dim=1)
        score_sre = score_sre.view(-1, self.num_choices)
        score_sre_soft = torch.softmax(score_sre, dim=1)
        ...
        select_index = self.get_ire_p_index(u_l, self.ire_num, sre_num)
        ...
        f_l = f_l.view(-1, self.ire_num, dim)
        f = self.self_attention_fuse(f_l)
        f = f.view(-1, self.num_choices, dim)
        f = self.w_9(f)
        f = self.relu(f)
        score_ire = self.w_10(f)
        score_ire = score_ire.view(-1, self.num_choices)
        score_ire_soft = torch.softmax(score_ire, dim=1)

        p_labels = p_labels.view(batch_size * self.num_choices, self.ere_num)
        if do_train:

            loss_ere = self.criterion(score_ere, answers)
            loss_sre = self.criterion(score_sre, answers)
            loss_ire = self.criterion(score_ire, answers)

            have_p_labels = have_p_labels.view(-1)
            z_l = z_l.view(-1, self.ere_num)
            active_choice = have_p_labels == 1
            # print('active_choice:', active_choice.shape)
            ...
            if pos_num == 0:
                ...
                loss_ere_p = self.listnet_loss(z_l_active, p_labels_active)

            u_l_active = u_l.view(batch_size, self.num_choices * sre_num)
            p_labels_sre_active = p_labels_sre.view(batch_size, self.num_choices * sre_num)

            loss_sre_p = self.cross_entropy_loss(u_l_active, p_labels_sre_active)
            ...
            if ire:
                loss += loss_ire
            if p:
                loss += loss_ere_p + loss_sre_p
            return loss
        else:
            p_labels = p_labels.view(-1, self.ere_num)
            return score_ere_soft, score_sre_soft, score_ire_soft, p_labels, p_ids, z_l, paragraphs

    def get_ire_p_index(self, scores, ire_num, sre_num):
        scores = scores.view(-1, sre_num)
        scores = scores.cpu().detach().numpy().tolist()
        select_index = []
        count = 0
        count1 = 0
        for score in scores:
            data = heapq.nlargest(ire_num, enumerate(score), key=lambda x: x[1])
            max_index, vals = zip(*data)
            max_index = list(max_index)
            select = []
            count1 += len(max_index)
            for i in range(sre_num):
                if i in max_index:
                    select.append(True)
                    count += 1
                else:
    ...
    def get_ir_feature(self, weight_set, word_set, max_result_num, max_seq_length, index_map, backgrounds, questions,
        ...
        for words, weights, background, question, option in zip(word_set, weight_set, backgrounds, questions, options):
            ...
            for content in contents:
                ...
                bm25_list = []
                for q in words:
                    score, _ = get_bm25_score_word(None, q, None, tokens_paragraph, index_map)
    ...
    def get_p_sre(self, z_ls, p_labels, paragraphs, scenarios, questions, options, batch_size,
                        top_k, word_sets, word_weights):

        z_ls = z_ls.view(batch_size * self.num_choices, self.ere_num)

        # print('z_ls:',z_ls.shape, 'word_weights:', word_weights.shape)
        # z_ls = z_ls.cpu().detach().numpy().tolist()
        step_size = self.ere_num
        i = 0
        idx_selext = []
        ...
        # print('p_labels end:',p_labels.shape)
